...
def main():
    ...
    train_df = pd.read_excel('image-set_features.xlsx', header=0)
    lb = LabelEncoder()
    ...
    # load all three bystandar test runs into list and concat for now
    test1_df = pd.read_excel('day1_test1_features.xlsx', header=0)
    lb = LabelEncoder()
    lb.fit(test1_df['label'])
    label_col = lb.fit_transform(test1_df['label'])
    test1_df['label'] = label_col
    test1_df.drop(labels=['center_deviation'], axis=1, inplace=True)
    ...
    ## output the labels from each session 
    
    #read the labels files, update with predictions, save as new file day1_test#_predictions
    test1_label_df = pd.read_csv('day1_test1_labels.csv')
    
    test1_label_df['Bystander Label'] = test1_df['label'].values
    test1_label_df.to_csv('day1_test1_predictions.csv',index=False)
    
    test2_label_df = pd.read_csv('day1_test2_labels.csv')
    
    test2_label_df['Bystander Label'] = test2_df['label'].values
    test2_label_df.to_csv('day1_test2_predictions.csv',index=False)
    
    test3_label_df = pd.read_csv('day1_test3_labels.csv')
    
    test3_label_df['Bystander Label'] = test3_df['label'].values
    test3_label_df.to_csv('day1_test3_predictions.csv',index=False)
    
    
    test_df = pd.concat([test1_df,test2_df,test3_df],ignore_index=True)
